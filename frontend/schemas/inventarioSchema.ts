...
export const registrarProductoSchema = z.object({
    ...
    precio: z.number({ error: 'El precio debe ser un número' }).min(0, 'El precio no puede ser negativo'),
    cantidad: z.number({ error: 'La cantidad debe ser un número' }).int().min(0, 'La cantidad no puede ser negativa'),
...
export const registrarMovimientoSchema = z.object({
    ...
    cantidad: z.number({ error: 'Debe ingresar una cantidad' }).int().positive('La cantidad debe ser mayor a 0'),
});
